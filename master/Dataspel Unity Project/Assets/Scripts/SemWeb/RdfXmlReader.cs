...
namespace SemWeb
{
    public class RdfXmlReader : RdfReader
    {
        ...
        public RdfXmlReader(Stream document)
            : this(new StreamReader(document))
        {
        }

        public RdfXmlReader(TextReader document, string baseUri)
            : this(document)
        {
            BaseUri = baseUri;
        }

        public RdfXmlReader(Stream document, string baseUri)
            : this(new StreamReader(document), baseUri)
        {
        }

        public RdfXmlReader(string file, string baseUri)
            : this(GetReader(file), baseUri)
        {
        }

        public RdfXmlReader(string file)
            : this(GetReader(file), "file:///" + file)
        {
        }
        ...
        private class XmlBaseAwareReader : XmlReader, IXmlLineInfo
        {
            ...
            public override bool Read()
            {
                ...
                if (temp)
                { // last element was an empty element
