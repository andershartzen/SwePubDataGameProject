...
namespace SemWeb.Inference
{
    ...
    public class Euler : Reasoner
    {
        ...
        public override void Select(SelectFilter filter, SelectableSource targetModel, StatementSink sink)
        {
            ...
            foreach (Statement s in filter)
            { // until we can operate on filter directly
                ...
                {
                    foreach (Statement h in ei.head)
                    { // better be just one statement
        ...
        {
            foreach (Statement s in new MemoryStore(body))
            { // make a copy since we remove statements from b within
